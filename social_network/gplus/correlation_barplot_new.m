%use function bar() in matlab to compare the medians of normal_simple and normal_ias for p=1,2 respectively
%for each Id generate one plot of such
p=genpath(pwd);
addpath(p);
...
%------------------------------load data-------------------------------------
	...
	median = zero(2,2);%median(1,:) : medians when p=1;median(2,:) : medians when p=2;
	...
	saveas(gca,sprintf('new_barplot_%s.eps',char(fileID)),'epsc');
end%for Id
